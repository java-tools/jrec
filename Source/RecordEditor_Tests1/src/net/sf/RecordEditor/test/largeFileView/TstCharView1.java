 ...
 *
 */
public class TstCharView1 extends TestCase {
	...
	private void tstA(String desc, int updateType, long seed, int firstLine, int numActions) {
		...
		tst1.doTest(new ViewTestData(schema, FileAndView.LARGE_CHAR_FILE, true, updateType), 
				desc, checkHeldLines, seed, firstLine, numActions, schema.getMaximumRecordLength());
		tst1.doTest(new ViewTestData(schema, FileAndView.LARGE_CHAR_FILE, false, updateType), 
				desc, checkHeldLines, seed, firstLine, numActions, schema.getMaximumRecordLength());
	}
}
