...
/**
 * This Class tests result in a view after updating the main
...
public class TstViewUpdateMain extends TestCase {
	...
	private  void doActualDelTestLine(boolean parent, boolean view, boolean delView, int[] delLineNums) {
		...
		int ll;
		fv = new TestDetails(parent, view, delView);
		for (int i = delLineNums.length - 1; i >= 0; i--) {
			ll = fixIdx(fv.baseFv.parentFile, delLineNums[i]);
			fv.baseFileToUse.deleteLine(fv.baseFv.viewFile.getLine(ll));
			fv.testFileToUse.deleteLine(fv.testFv.parentFile.getLine(ll));

			checkFV(id + "Test 3f: " + ll + " ~ ", fv);
		}

		fv = new TestDetails(parent, view, delView);
		for (int i = delLineNums.length - 1; i >= 0; i--) {
			ll = fixIdx(fv.baseFv.parentFile, delLineNums[i]);
			fv.baseFileToUse.deleteLine(ll);
			fv.testFileToUse.deleteLine(ll);

			checkFV(id + "Test 3g: " + ll + " ~ ", fv);
		}
