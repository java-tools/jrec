...
import net.sf.RecordEditor.utils.swing.treeCombo.TreeComboFileSelect2;
...
//	private FileChooser  copybookFile;
    private TreeComboFileSelect	 copybookDir;
    private TreeComboFileSelect2 copybookFile;
...
//    private QuoteCombo quote;
	 ...
	 */
	public void addLayoutSelection(BasePanel pnl, JPanel goPanel, 	TreeComboFileSelect2 layoutFile) {
	...
	private void addLayoutSelection(BasePanel pnl, JPanel goPanel,
			JButton layoutCreate1, JButton layoutCreate2, TreeComboFileSelect2 layoutFile) {
  	    setupFields();
...
//	    });
	    ...
	    if (layoutFile != null)  {
			...
			pnl.addLine("Copybook Directory", copybookDir);
			pnl.addLine("Copybook", copybookFile);
	    } else {
...
//            quote.setSelectedIndex(getIntToken(t));
	...
	private void setupFields() {
		...
		if (copybookFile == null) {
			...
			copybookDir  = new TreeComboFileSelect(true, true, true, itms);
			copybookFile = new TreeComboFileSelect2(
					true, false, new ArrayList<FileTreeComboItem>(), updateTxt, false, (JButton[]) null);

			s = Common.OPTIONS.DEFAULT_COPYBOOK_DIRECTORY.get();
			if (itms.size() >= 0) {
				s = itms.get(0).getFullname();
			}
			copybookDir.setText(s);

			copybookDir.addFileChangeListner(new ChangeListener() {
