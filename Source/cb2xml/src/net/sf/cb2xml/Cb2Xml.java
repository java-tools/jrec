...
public class Cb2Xml {
	...
	public static Document convertToXMLDOM(File file) {
		return convert(file, null, false);
	}
	
	public static String convertToXMLString(File file) {
		Document document = convert(file, null, false);
	...
	public static Document convertToXMLDOM(File file, boolean debug) {
		return convert(file, null, debug);
	}
	
	public static String convertToXMLString(File file, boolean debug) {
		Document document = convert(file, null, debug);
		return XmlUtils.domToString(document).toString();
	}	
	
	public static Document convertToXMLDOM(InputStream is) {
		return convert(null, is, false);
	}
	
	public static String convertToXMLString(InputStream is) {
		Document document = convert(null, is, false);
		return XmlUtils.domToString(document).toString();
	}

	// overloaded methods for debug mode
	public static Document convertToXMLDOM(InputStream is, boolean debug) {
		return convert(null, is, debug);
	}
	
	public static String convertToXMLString(InputStream is, boolean debug) {
		Document document = convert(null, is, debug);
		return XmlUtils.domToString(document).toString();
	}	

	
	private static Document convert(File file, InputStream is, boolean debug) {
