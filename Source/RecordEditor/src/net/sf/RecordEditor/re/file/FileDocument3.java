...
public class FileDocument3 extends AbstractDocument implements Document, Element, DocumentUpdateListner {
   ...
   public static final String tabSizeAttribute = "tabSize";
     ...
     */
    public FileDocument3(FileView f, @SuppressWarnings("rawtypes") IDataStoreText c) {
...
//		return 0;
	...
	public Element getElement(int index) {
		...
		return new LineElement(this, dsContent.getLinePosition(index));
	}
	...
	public void fireUpdate(int type, int firstLine, int lastLine) {
		...
		IDataStorePosition lastPos = dsContent.getLinePosition(lastLine, false);
		if (firstLine < 0 || lastLine < 0) {
			where = 0;
			count = dsContent.length();
		} else {
			IDataStorePosition startPos = dsContent.getLinePosition(firstLine, false);
...
//			count = lastPos.getOffset() + lastPos.getLine().getFullLine().length() - where + 2;
	...
	private static class LineElement implements Element {
		...
		@Override
		public int getEndOffset() {
			return pos.getLineStart() + pos.getLine().getFullLine().length() + 1;
		}
	...
	private static class ColorLineElement extends LineElement {
		...
		private int getSchemaIndex() {
			...
			return v.getLine(pos.getLineNumber()).getPreferredLayoutIdx();
		}
	}
	...
	private static class FieldElement implements Element {
		...
		public FieldElement(ColorLineElement parent, IFieldDetail field, int idx) {
			...
			this.field = field;
			this.index = idx;
			Object[] xx = {StyleConstants.Foreground};
