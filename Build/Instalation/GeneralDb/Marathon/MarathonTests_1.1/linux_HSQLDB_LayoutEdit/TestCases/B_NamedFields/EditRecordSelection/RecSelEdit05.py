...
##		select('ChildRecordsJTbl', '')
		...
		assert_p('RecordSelectionJTbl', 'Content', '[[, , fld 21, =, 21], [, ' + commonBits.fl('And') + ', fld 21, =, 21aa], [' + commonBits.fl('Or') + ', , fld 22, =, 22]]')
		select('RecordSelectionJTbl', 'cell:' + commonBits.fl('Field Value') + ',2()')
...
##		select('ChildRecordsJTbl', '')
		...
		assert_p('RecordSelectionJTbl1', 'Content', '[[, , fld 21, =, 21], [, ' + commonBits.fl('And') + ', fld 21, =, 21aa], [' + commonBits.fl('Or') + ', , fld 22, =, 22]]')
		assert_p('RecordSelectionJTbl1', 'Content', '[[, , fld 21, =, 21], [, ' + commonBits.fl('And') + ', fld 21, =, 21aa], [' + commonBits.fl('Or') + ', , fld 22, =, 22]]')
