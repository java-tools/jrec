...
##		select('Table2', 'rows:[3,4,5,6,7,8,9,10],columns:[1 - 8|KEYCODE-NO]')
		...
		select('Table', 'cell:         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',3([B@16ce64e)')
##		assert_p('Table', 'Content', '[[[B@16269c], [[B@1a95cf6], [[B@844c3d], [[B@16ce64e], [[B@1188793], [[B@1e5d16d], [[B@188a478], [[B@14fb35d]]')
		select('Table', 'cell:         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',1([B@1a95cf6)')
		assert_p('Table', 'Text', 'f6f3f6f0f4f8f0f8020c0040118c170c000000001c00000000487c', '         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',3')
		select('Table', 'cell:         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',2([B@844c3d)')
		assert_p('Table', 'Text', 'f6f9f6f9f4f1f5f8020c0040118c280c000000001c00000000501c', '         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',2')
		select('Table', 'cell:         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',3([B@16ce64e)')
		assert_p('Table', 'Text', 'f6f2f6f8f4f6f7f1020c0040118c685c000000001c00000006999c', '         +         1         +         2         +         3|' + commonBits.fl('Hex (1 Line)') + ',4')
	close()
