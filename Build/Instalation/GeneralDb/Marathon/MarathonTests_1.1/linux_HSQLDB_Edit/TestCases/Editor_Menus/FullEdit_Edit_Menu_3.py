useFixture(RecordEditor)

def test():
	from Modules import commonBits
	java_recorded_version = '1.5.0_11'

### Check out ....
	if window('Record Editor'):
		select('FileChooser', commonBits.sampleDir() + 'Ams_LocDownload_20041228.txt')
		commonBits.setRecordLayout(select, 'ams Store')
		click(commonBits.fl('Edit') + '1')
		select('Table', 'rows:[1,2,3,4,5,6,7,8,9],columns:[10 - 35|Loc Name]')
		select_menu(commonBits.fl('Data') + '>>' + commonBits.fl('Table View #{Selected Records#}'))
##		select('Table2', 'rows:[1,2,3,4,5,6,7,8,9],columns:[10 - 35|Loc Name]')
		select('Table', 'rows:[2,3,4],columns:[8 - 2|Loc Type]')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Cut Record#{s#}'))
		select('Table', 'cell:10 - 35|Loc Name,2(Blacktown)')
		rightclick('Table', '10 - 35|Loc Name,2')
		select('Table', 'cell:10 - 35|Loc Name,0(VIC West Ad Support)')
		rightclick('Table', '10 - 35|Loc Name,0')
		select_menu(commonBits.fl('Edit Record'))
##		select('Table1', 'cell:10 - 35|Loc Name,0(VIC West Ad Support)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(VIC West Ad Support)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',3(VIC West Ad Support)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',5(Lot 2 Little Boundary Rd)')
		assert_p('Table', 'Content', '[[Brand Id, 1, 3, TAR, TAR], [Loc Nbr, 4, 4, 5850, 5850], [Loc Type, 8, 2, DC, DC], [Loc Name, 10, 35, VIC West Ad Support, VIC West Ad Support], [Loc Addr Ln1, 45, 40, , ], [Loc Addr Ln2, 85, 40, Lot 2 Little Boundary Rd, Lot 2 Little Boundary Rd], [Loc Addr Ln3, 125, 35, Laverton, Laverton], [Loc Postcode, 160, 10, 3028, 3028], [Loc State, 170, 3, VIC, VIC], [Loc Actv Ind, 173, 1, A, A]]')
		select('Table', 'cell:' + commonBits.fl('Data') + ',5(Lot 2 Little Boundary Rd)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(NSW North Sydney Ad Support)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',3(NSW North Sydney Ad Support)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		assert_p('Table', 'Content', '[[Brand Id, 1, 3, TAR, TAR], [Loc Nbr, 4, 4, 5853, 5853], [Loc Type, 8, 2, DC, DC], [Loc Name, 10, 35, NSW North Sydney Ad Support, NSW North Sydney Ad Support], [Loc Addr Ln1, 45, 40, , ], [Loc Addr Ln2, 85, 40, , ], [Loc Addr Ln3, 125, 35, , ], [Loc Postcode, 160, 10, , ], [Loc State, 170, 3, , ], [Loc Actv Ind, 173, 1, A, A]]')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5033)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',1(5033)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Marayong)')
		assert_p('Table', 'Content', '[[Brand Id, 1, 3, TAR, TAR], [Loc Nbr, 4, 4, 5033, 5033], [Loc Type, 8, 2, ST, ST], [Loc Name, 10, 35, Blacktown, Blacktown], [Loc Addr Ln1, 45, 40, Marayong, Marayong], [Loc Addr Ln2, 85, 40, Dock 2, 11 Melissa Place, Dock 2, 11 Melissa Place], [Loc Addr Ln3, 125, 35, Marayong, Marayong], [Loc Postcode, 160, 10, 2148, 2148], [Loc State, 170, 3, NSW, NSW], [Loc Actv Ind, 173, 1, A, A]]')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Marayong)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Building B,  Portside DC)')
		assert_p('Table', 'Content', '[[Brand Id, 1, 3, TAR, TAR], [Loc Nbr, 4, 4, 5035, 5035], [Loc Type, 8, 2, ST, ST], [Loc Name, 10, 35, Rockdale, Rockdale], [Loc Addr Ln1, 45, 40, Building B,  Portside DC, Building B,  Portside DC], [Loc Addr Ln2, 85, 40, 2-8 Mc Pherson Street, 2-8 Mc Pherson Street], [Loc Addr Ln3, 125, 35, Botany, Botany], [Loc Postcode, 160, 10, 2019, 2019], [Loc State, 170, 3, NSW, NSW], [Loc Actv Ind, 173, 1, A, A]]')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5035)')
		assert_p('Table', 'Text', '5035', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5035)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5037)')
		assert_p('Table', 'Text', '5037', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Westfield Shoppingtown)')
		assert_p('Table', 'Content', '[[Brand Id, 1, 3, TAR, TAR], [Loc Nbr, 4, 4, 5037, 5037], [Loc Type, 8, 2, ST, ST], [Loc Name, 10, 35, Miranda, Miranda], [Loc Addr Ln1, 45, 40, Westfield Shoppingtown, Westfield Shoppingtown], [Loc Addr Ln2, 85, 40, Cnr. Urunga Pde & The Kingsway, Cnr. Urunga Pde & The Kingsway], [Loc Addr Ln3, 125, 35, Miranda, Miranda], [Loc Postcode, 160, 10, 2228, 2228], [Loc State, 170, 3, NSW, NSW], [Loc Actv Ind, 173, 1, A, A]]')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Westfield Shoppingtown)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		select('Table', 'cell:10 - 35|Loc Name,2(Blacktown)')
		select('Table', 'cell:10 - 35|Loc Name,2(Blacktown)')
		assert_p('Table', 'RowCount', '6')
		select('Table', 'cell:10 - 35|Loc Name,4(Miranda)')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Paste Record#{s#}'))

		select('Table', 'cell:10 - 35|Loc Name,4(Miranda)')
		rightclick('Table', '10 - 35|Loc Name,4')
		select_menu(commonBits.fl('Edit Record'))
##		select('Table1', 'cell:10 - 35|Loc Name,4(Miranda)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5037)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',1(5037)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Westfield Shoppingtown)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',4(Westfield Shoppingtown)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Westfield Shoppingtown)')
		click('Right')
		  
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5052)')
		assert_p('Table', 'Text', '5866', commonBits.fl('Data') + ',1')
   ### was 5052
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(WA Ad Support)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(WA Ad Support)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5015)')
		assert_p('Table', 'Text', '5015', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(Bankstown)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(Bankstown)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5019)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',1(5019)')
   ### was 5052
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Penrith)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Penrith)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		select('Table', 'cell:10 - 35|Loc Name,5(WA Ad Support)')
		select('Table', 'cell:10 - 35|Loc Name,5(WA Ad Support)')
		assert_p('Table', 'RowCount', '9')
   ### was 6
		select('Table', 'cell:10 - 35|Loc Name,6(Bankstown)')
		select('Table', 'rows:[2,3,4],columns:[10 - 35|Loc Name]')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Delete Record#{s#}'))
		select('Table', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		assert_p('Table', 'RowCount', '6')
		select('Table', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		rightclick('Table', '10 - 35|Loc Name,1')
		select_menu(commonBits.fl('Edit Record'))
##		select('Table1', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		assert_p('Table', 'Text', '5853', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5866)')
		assert_p('Table', 'Text', '5866', commonBits.fl('Data') + ',1')
 ### 5033
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5866)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5015)')
		assert_p('Table', 'Text', '5015', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',5(Unit 2, 39-41 Allingham Street)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',5(Unit 2, 39-41 Allingham Street)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5019)')
		assert_p('Table', 'Text', '5019', commonBits.fl('Data') + ',1')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Penrith)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',4(Penrith)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		select('Table', 'cell:10 - 35|Loc Name,2(WA Ad Support)')
		select('Table', 'cell:10 - 35|Loc Name,2(WA Ad Support)')
		assert_p('Table', 'RowCount', '6')
		select('Table', 'cell:10 - 35|Loc Name,3(Bankstown)')
		select('Table', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Copy Record#{s#}'))
		select('Table', 'cell:8 - 2|Loc Type,3(ST)')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Paste Record#{s#} Prior'))

		select('Table', 'cell:4 - 4|Loc Nbr,2(5866)')
		rightclick('Table', '4 - 4|Loc Nbr,2')
		select_menu(commonBits.fl('Edit Record'))
##		select('Table1', 'cell:4 - 4|Loc Nbr,2(5866)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5866)')
		assert_p('Table', 'Text', 'DC', commonBits.fl('Data') + ',2')
  ### was ST
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5866)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		assert_p('Table', 'Text', '5853', commonBits.fl('Data') + ',1')
 ### was 5035
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5853)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5015)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',1(5015)')
  ### was 5037
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(Bankstown)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',3(Bankstown)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5019)')
		assert_p('Table', 'Text', 'cell:' + commonBits.fl('Data') + ',1(5019)')
 ### was 5019 ???
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5019)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		select('Table', 'cell:10 - 35|Loc Name,5(Penrith)')
		select('Table', 'cell:10 - 35|Loc Name,5(Penrith)')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Paste Record#{s#}'))

		select('Table', 'cell:10 - 35|Loc Name,5(Penrith)')
		rightclick('Table', '10 - 35|Loc Name,5')
		select_menu(commonBits.fl('Edit Record'))
##		select('Table1', 'cell:10 - 35|Loc Name,5(Penrith)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5052)')
		assert_p('Table', 'Text', '5019', commonBits.fl('Data') + ',1')
 ### was 5052
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5019)')
		click('Right')
		select('Table', 'cell:' + commonBits.fl('Data') + ',1(5052)')
		assert_p('Table', 'Text', '5853', commonBits.fl('Data') + ',1')
 ### was 5052
		select('Table', 'cell:' + commonBits.fl('Data') + ',2(DC)')
		select('Table', 'cell:' + commonBits.fl('Data') + ',2(DC)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		select('Table', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		select('Table', 'rows:[1,2,3,4,5],columns:[10 - 35|Loc Name]')
		select_menu(commonBits.fl('Edit') + '>>' + commonBits.fl('Delete Record#{s#}'))
		select('Table', 'cell:10 - 35|Loc Name,1(NSW North Sydney Ad Support)')
		assert_p('Table', 'RowCount', '3')
  ### was 6
		select('Table', 'cell:10 - 35|Loc Name,0(VIC West Ad Support)')
		select('Table', 'cell:10 - 35|Loc Name,0(VIC West Ad Support)')
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')
		#click('WindowsInternalFrameTitlePane', 856, 12)
		commonBits.closeWindow(click)
		##click('BasicInternalFrameTitlePane$NoFocusButton2')

		if window(commonBits.fl('Save Changes to file: ' + commonBits.sampleDir() + 'Ams_LocDownload_20041228.txt')):
			click('No')
		close()
	close()
