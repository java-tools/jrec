...
public class TstStandardParser extends TestCase {
	...
	public void testSetField() {
		...
		for (int i = 0; i < lines1.length - 1; i++) {
			for (j = 0; j < 3; j++) {
				s = p.setField(j, Type.NT_TEXT, lines1[i], ",", "'", 
						p.getField(j, lines1[i], ",", "'"));
				assertEquals("Error in " + i +":" + j + "a got " + s, lines1[i], s);
				
				s = p.setField(j, Type.NT_TEXT, lines2[i], "|", "'", 
						p.getField(j, lines2[i], "|", "'"));
				assertEquals("Error in " + i +":" + j + "b got " + s, lines2[i], s);

				s = p.setField(j, Type.NT_TEXT, lines0[i], ",", "'", 
						p.getField(j, lines0[i], ",", ""));
	...
	public void testSetField2() {
		...
		for (int i = 0; i < lines1.length - 1; i++) {
			for (j = 0; j < 3; j++) {
				s = p.setField(j, Type.NT_NUMBER, lines1[i], ",", "'", 
						p.getField(j, lines1[i], ",", "'"));
				assertEquals("Error in " + i +":" + j + "a got " + s, lines1[i], s);
				
				s = p.setField(j, Type.NT_NUMBER, lines2[i], "|", "'", 
						p.getField(j, lines2[i], "|", "'"));
				assertEquals("Error in " + i +":" + j + "b got " + s, lines2[i], s);

				s = p.setField(j, Type.NT_NUMBER, lines0[i], ",", "'", 
						p.getField(j, lines0[i], ",", ""));
				assertEquals("Error in " + i +":" + j + "c got " + s, lines0[i], s);
			}
		}
	...
	public void testSetField3() {
			...
			for (int i = 0; i < lines1.length - 1; i++) {
				for (j = 0; j < 3; j++) {
					before = p.getField(j, lines1[i], ",", "'");
					s = p.setField(j, Type.NT_TEXT, lines1[i], ",", "'", before);
					
					if (before.indexOf(",") == 0) {
						after = p.getField(j, s, ",", "");
					...
					before = p.getField(j, lines2[i], "|", "'");
					s = p.setField(j, Type.NT_TEXT, lines2[i], "|", "'", before);
					...
					before = p.getField(j, lines0[i], ",", "");
					s = p.setField(j, Type.NT_TEXT, lines0[i], ",", "'", before);
					after = p.getField(j, s,  ",", "");
					System.out.println(i + ", " + j + " > " + before + " => " + after);
